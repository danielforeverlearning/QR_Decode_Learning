...
public class BigInteger_Berlekamp_Welch_algorithm {
    ...
    public BigInteger_Berlekamp_Welch_algorithm(int gf_val, int[] temp_b, int temp_e_count) throws  Exception
    {
        GF = gf_val;
        ...
        answer_matrix = new BigInteger[b.length];
        //it holds values -b[1]a[1]^2 .....-b[recv_max_index]a[recv_max_index]^2
        for (int ii=1; ii <= recv_max_index; ii++)
        {
            BigInteger temp = BigInteger.valueOf(a[ii]);
            temp = temp.multiply(temp);
            temp = temp.multiply(BigInteger.valueOf(b[ii]));
            temp = temp.negate();
            answer_matrix[ii] = GF_value(temp);
        }
        
        
        matrix = new BigInteger[b.length][b.length];
        //so do not use matrix[0][anything] or matrix[anything][0] just try to mark it as not used
        for (int ii=0; ii <= recv_max_index; ii++)
        {
            matrix[0][ii] = BigInteger.valueOf(Integer.MAX_VALUE);
            matrix[ii][0] = BigInteger.valueOf(Integer.MAX_VALUE);
        }
        ...
        for (int row=1; row <= recv_max_index; row++)
        {
            BigInteger b_row = BigInteger.valueOf(b[row]);
            BigInteger a_row = BigInteger.valueOf(a[row]);
            BigInteger neg1  = BigInteger.ONE.negate();
            
            matrix[row][1] = b_row;
            matrix[row][2] = GF_value(b_row.multiply(a_row));
            matrix[row][3] = GF_value(neg1);
            matrix[row][4] = GF_value(neg1.multiply(a_row));
            matrix[row][5] = GF_value(neg1.multiply(GF_pow(a_row,2)));
            matrix[row][6] = GF_value(neg1.multiply(GF_pow(a_row,3)));
            for (int col=7; col <= recv_max_index; col++)
                matrix[row][col] = GF_value(neg1.multiply(GF_pow(a_row,col-3)));
        }//for row
    }//constructor
    
    
    private BigInteger GF_pow(BigInteger num, int exponent) throws Exception
    {
        /******
        int temp = 1;
    ...
    private BigInteger GF_value(BigInteger temp_decimal)
    {
        /*****
        int answer = temp_decimal % BigInteger.valueOf(GF);
        if (answer < 0)
            answer += GF;
        return answer;
        *****/
        
        BigInteger answer = temp_decimal.remainder(BigInteger.valueOf(GF));
        int compare = answer.compareTo(BigInteger.valueOf(0));
        if (compare == -1)
            answer = answer.add(BigInteger.valueOf(GF));
        return answer;
    }//GF_value
    ...
    public void Debug_Print()
    {
        ...
        for (int row=1; row <= recv_max_index; row++)
        {
            ...
            for (int col=1; col <= recv_max_index; col++)
            {
                String tempstr = matrix[row][col].toString();
                if (tempstr.length() == 3)
                    tempstr = "  " + tempstr;
    ...
    public void MultiplyAndAdd(int row_mult, BigInteger mult_val, int row_add)
    {
        for (int col=1; col <= recv_max_index; col++)
        {
            BigInteger temp = matrix[row_mult][col];
            temp = temp.multiply(mult_val);
            temp = temp.add(matrix[row_add][col]);
            temp = GF_value(temp);
            matrix[row_add][col] = temp;
        }

        BigInteger temp = answer_matrix[row_mult];
        temp = temp.multiply(mult_val);
        temp = temp.add(answer_matrix[row_add]);
        temp = GF_value(temp);
        answer_matrix[row_add] = temp;
    }//MultiplyAndAdd
    
    
    public void Multiply_Row(int row, BigInteger val)
    {
        for (int col=1; col <= recv_max_index; col++)
        {
            BigInteger temp = matrix[row][col];
            temp = temp.multiply(val);
            temp = GF_value(temp);
            matrix[row][col] = temp;
        }

        BigInteger temp = answer_matrix[row];
        temp = temp.multiply(val);
        temp = GF_value(temp);
    ...
    public void Add_RowA_By_RowB(int RowA, int RowB)
    {
        for (int col=1; col <= recv_max_index; col++)
        {
            BigInteger Aval = matrix[RowA][col];
            BigInteger Bval = matrix[RowB][col];
            BigInteger temp = Aval.add(Bval);
            temp = GF_value(temp);
            matrix[RowA][col] = temp;
        }

        BigInteger Aval = answer_matrix[RowA];
        BigInteger Bval = answer_matrix[RowB];
        BigInteger temp = Aval.add(Bval);
        temp = GF_value(temp);
    ...
    {   
        ...
        while (str.equals("x") == false)
        {
            ...
            if (temp[0].equals("m"))
                Multiply_Row(Integer.parseInt(temp[1]), BigInteger.valueOf(Integer.parseInt(temp[2])));
            else if (temp[0].equals("a"))
                Add_RowA_By_RowB(Integer.parseInt(temp[1]), Integer.parseInt(temp[2]));
            else if (temp[0].equals("ma"))
                MultiplyAndAdd(Integer.parseInt(temp[1]), BigInteger.valueOf(Integer.parseInt(temp[2])), Integer.parseInt(temp[3]));
    ...
    private boolean Check_Identity_Matrix()
    {
        for (int row=1; row <= recv_max_index; row++)
        {
            for (int col=1; col <= recv_max_index; col++)
            {
                if (matrix[row][col].compareTo(BigInteger.ONE)!=0 && row==col)
                    return false;
                else if (matrix[row][col].compareTo(BigInteger.ZERO)!=0 && row!=col)
                    return false;
            }
        }
        return true;
    ...
    *****/
    ...
    private int FindRowBelowDiagWhoseColCellIs_1(int diag)
    {
        ...
        {
            if (matrix[row][diag].compareTo(BigInteger.ONE) == 0)
                return row;
        }
        return -1;
    ...
    *****/
    ...
    private boolean GF256_Make_Column_Good_1_And_0s(int ii)
    {
        ...
        for (int row=1; row <= recv_max_index; row++)
        {
            if (row==ii && matrix[row][ii].compareTo(BigInteger.ONE) == 0)
                good = true;
            else if (row!=ii && matrix[row][ii].compareTo(BigInteger.ZERO) == 0)
                good = true;
            else
            {
                good = false;
        ...
        for (int row=1; row <= recv_max_index; row++)
        {
            if (matrix[row][ii].compareTo(BigInteger.ZERO) == 0)
                zero_count++;
        }
        if (zero_count == recv_max_index)
            return false; //can not make it good
