 ...
 * sizes of the character count indicators for each mode and version. For
 * example, if encoding HELLO WORLD in a version 1 QR code in alphanumeric mode,
 * the character count indicator must be 9 bits long. The character count of
 * HELLO WORLD is 11. In binary, 11 is 1011. Pad it on the left to make it 9
 ...
 *
 * Versions 27 through 40 Numeric mode: 14 bits Alphanumeric mode: 13 bits Byte
 * mode: 16 bits Japanese mode: 12 bits
 *
 *
 *
 * Alphanumeric Mode Encoding: For alphanumeric mode encoding, I will use the
 * example input of HELLO WORLD. For this example, I will use version 1.
 * Remember, alphanumeric mode can only encode uppercase letters, not lowercase.
 * Refer to the alphanumeric table for a list of the characters that can be
 * encoded in alphanumeric mode.
 *
 * (1)Break up into pairs First, break up the string into pairs of characters:
 * HE, LL, O , WO, RL, D
 *
 * (2)Create a binary number for each pair For alphanumeric mode, each
 * alphanumeric character is represented by a number. Please refer to the
 * alphanumeric table to find these numbers. The column on the left shows the
 * alphanumeric character, and the column on the right shows the number that
 * represents it. For each pair of characters, get the number representation
 * (from the alphanumeric table) of the first character and multiply it by 45.
 ...
 * H → 17 E → 14
 *
 * Following the steps in the previous paragraph, multiply the first number by
 * 45, then add that to the second number: (45 * 17) + 14 = 779
 *
 * Now convert that number into an 11-bit binary string, padding on the left
 * with 0s if necessary.
 *
 * 779 → 01100001011
 *
 * If you are encoding an odd number of characters, as we are here, take the
 * numeric representation of the final character and convert it into a 6-bit
 * binary string.
 *
 * Next: Finish the Data Encoding Step Follow the instructions on the data
 * encoding page to add any remaining bits as necessary.
 * **********************************************
 */
public class DataCodewordsDecoder {
            ...
            while (true) {
                ...
                if (getIndicator) {
                    ...
                    if (startOfByte.length() == 0) //getIndicator on byte boundary
                    {
                        ...
                        if (encoding == ENCODING_INDICATOR.END_OF_MESSAGE) {
                            break;
                        } else if ((encoding != ENCODING_INDICATOR.BYTE) && (encoding != ENCODING_INDICATOR.ALPHANUMERIC)) {
                    ...
                    else //getIndicator startOfByte has partial byte
                    {
                        if (startOfByte.length() == 4) { //getIndicator startOfByteLen == 4
                            ...
                            /**
                             * **********************************
                             * length field ------------- Versions 1 through 9
                             * Numeric mode: 10 bits Alphanumeric mode: 9 bits
                             * Byte mode: 8 bits Japanese mode: 8 bits
                             * **********************************
                             */
                            int need_bit_len = 0;
                    ...
                    if (length > 0) {
                        if (encoding == ENCODING_INDICATOR.BYTE) {
                            ...
                            /**
                             * **********************************
                             * Numeric Mode Encoding To illustrate numeric mode
                             * encoding, the example input is 8675309
                             *
                             * Step 1: Break String Up Into Groups of Three To
                             * encode a string of digits in numeric mode, first
                             * split the string into groups of 3 digits. If the
                             * string's length is not a multiple of 3, the final
                             * group of digits will have to be only 1 or 2
                             * numbers long.
                             *
                             * After splitting into groups of three: 867 530 9
                             *
                             * Step 2: Convert each group into binary Now treat
                             * each group of digits as a 3 digit number (or
                             * fewer than 3, if the final group is 2 or 1 digits
                             * long). Convert that 3-digit number into 10 binary
                             * bits. If a group starts with a 0, it should be
                             * interpreted as a 2-digit number and you should
                             * convert it to 7 binary bits, and if there are 2
                             * 0s at the beginning of a group, it should be
                             * interpreted as a 1-digit number and you should
                             * convert it to 4 binary bits. Similarly, if the
                             * final group consists of only 2 digits, you should
                             * convert it to 7 binary bits, and if the final
                             * group consists of only 1 digit, you should
                             * convert it to 4 binary bits.
                             *
                             * Converting to binary: 867 → 1101100011
                             *
                             * 530 → 1000010010
                             *
                             * 9 → 1001 **********************************
                            ...
                            if (length >= 3) {
                                ...
                                String str10bits = "";
                                if (startOfByte.length() == 10) {
                                    str10bits = startOfByte;
                                    startOfByte = "";
                                } else {
    ...
    public boolean Decode_Correct_Sequential_Codewords(
    ...
    public boolean Decode_Correct_Sequential_Codewords(ArrayList<Integer> block1, ArrayList<Integer> block2) {
        ArrayList<Integer> data = new ArrayList<Integer>();
        data.addAll(block1);
        data.addAll(block2);
        boolean result = decode(data);
    ...
    public boolean Decode_Correct_Sequential_Codewords(String input_filename) {
        ...
        try {
            ...
            while (true)
            {
                ...
                data.add(Integer.parseInt(inputData));
            }
        } catch (FileNotFoundException ex) {
            System.out.println("Decode_Correct_Sequential_Codewords: FileNotFoundException occurred!");
            ex.printStackTrace();
            return false;
            ...
            System.out.println("Decode_Correct_Sequential_Codewords: Exception occurred!");
            ex.printStackTrace();
            return false;
        } 
        boolean result = decode(data);
